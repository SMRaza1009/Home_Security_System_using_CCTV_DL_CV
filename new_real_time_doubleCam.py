...
net2 = cv2.dnn.readNet('/home/syed/Desktop/Single-Multiple-Custom-Object-Detection-master/1. Project - Custom Object Detection/yolov3_best.weights', '/home/syed/Desktop/Single-Multiple-Custom-Object-Detection-master/1. Project - Custom Object Detection/yolov3.cfg')
...
filename =  'Real-time_videos/'+str(y)+'.avi'
res = '720p'
...
output1 = cv2.VideoWriter(filename, get_video_type(filename), 25, get_dims(cap, res))
output2 = cv2.VideoWriter(filename, get_video_type(filename), 25, get_dims(cap2, res))
...
while True:
  ...
  output_layers_names2 = net2.getUnconnectedOutLayersNames()
  layerOutputs = net.forward(output_layers_names)
  layerOutputs2 = net2.forward(output_layers_names2)
  boxes = []
  confidences = []
  class_ids = []
  ...
  if len(indexes)>0:
     for i in indexes.flatten():
         ...
         if confidence > str(0.5) and label == "gun"  :
           ...
           cv2.imwrite(os.path.join(path , val),img)
           play(sound)
  ...
  if len(indexes)>0:
     for i in indexes.flatten():
         ...
         if confidence > str(0.5) and label == "gun"  :
           val2 =  'KHI-MADRAS CHOWK-A406-Gun-Detected'+ str(x)+'.png'
           cv2.imwrite(os.path.join(path , val2),img2)
           play(sound)
